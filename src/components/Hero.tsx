      ...
      <div className="relative max-w-6xl mx-auto px-4 md:px-8 py-12 md:py-24 grid md:grid-cols-[1fr_auto] gap-10 items-center">
        <div className="space-y-6 md:space-y-8">
          ...
          <h1 className="font-display text-[13vw] md:text-8xl lg:text-9xl leading-[0.9]">
            ...
            <br />
            GW{" "}
            <span className="inline-block bg-ink text-paper px-3 md:px-5 -rotate-2">
              DIKA
            </span>
          </h1>
        ...
        <div className="relative justify-self-center md:justify-self-end">
          ...
          {/* Badge moved below card so it doesn't cover location text */}
          <div className="absolute -bottom-9 -left-3 nb-border-thick bg-pink text-white rounded-lg px-3 py-2 font-display text-sm md:text-base nb-shadow-sm -rotate-6">
            GANTENG ✓
          </div>
        </div>
      </div>
          ...
          {Array.from({ length: 2 }).map((_, k) => (
            ...
            >
              {[
                ...
                <span
                  key={i}
                  className="bg-paper text-ink px-3 py-0.5 nb-border rounded"
                >
                  {t}
                </span>
              ))}
...
function StatBox({
  ...
  return (
    <div
      className={`nb-border-thick rounded-lg p-2 md:p-3 nb-shadow-sm ${tone}`}
    >
